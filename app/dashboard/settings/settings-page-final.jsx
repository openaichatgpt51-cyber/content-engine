...
function SettingsFlow() {
  ...
  const [brandImages, setBrandImages] = useState([])           // uploaded URLs
  const [visualStyle, setVisualStyle] = useState('')           // analyzed description
  ...
  async function uploadBrandImage(file) {
    if (!file) return
    if (brandImages.length >= 5) { setImageError('Up to 5 images'); return }
    setImageUploading(true)
    setImageError('')
    try {
      const { data: { user } } = await supabase.auth.getUser()
      const ext  = file.name.split('.').pop()
      const path = `${user.id}/brand-${Date.now()}.${ext}`
      const { error: uploadErr } = await supabase.storage.from('post-images').upload(path, file, { upsert: true })
      if (uploadErr) throw uploadErr
      const { data: pub } = supabase.storage.from('post-images').getPublicUrl(path)
      setBrandImages(imgs => [...imgs, pub.publicUrl])
    } catch (err) {
  ...
  function removeBrandImage(url) {
    setBrandImages(imgs => imgs.filter(u => u !== url))
  }

  async function analyzeBrandImages() {
    if (!brandImages.length) return
    setAnalyzing(true)
    setImageError('')
    try {
      const res = await fetch('/api/analyze-brand-images', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ imageUrls: brandImages }),
      })
      const body = await res.json()
      if (!res.ok) throw new Error(body.error || 'Analysis failed')
      setVisualStyle(body.description)
    } catch (err) {
  ...
  async function loadData() {
    ...
    if (profile) {
      ...
      setBrandImages(profile.brand_image_urls || [])
      setVisualStyle(profile.visual_style_description || '')
    }
  }
    ...
    <div style={{ padding: '32px 36px', flex: 1, maxWidth: 800 }}>
      ...
      {activeTab === 'brandvoice' && (
        <form className="animate-in" onSubmit={saveBrand}>
          ...
          <BrandSection label="Brand Images" hint="Upload a few images that represent your visual style — the AI will analyze it and apply it to generated images" index={5}>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginBottom: 12 }}>
              {brandImages.map(url => (
                <div key={url} style={{ position: 'relative', width: 80, height: 80 }}>
                  <img src={url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', borderRadius: 'var(--radius-sm)', border: '1px solid var(--fog-60)' }} />
                  <button
                    type="button"
                    onClick={() => removeBrandImage(url)}
                    style={{
                      position: 'absolute', top: -6, right: -6, width: 20, height: 20, borderRadius: '50%',
                      background: 'var(--ink)', color: 'white', border: 'none', fontSize: '0.7rem', cursor: 'pointer',
                    }}
                  >×</button>
                </div>
              ))}
              {brandImages.length < 5 && (
                <label style={{
                  width: 80, height: 80, borderRadius: 'var(--radius-sm)', border: '1.5px dashed var(--fog-60)',
                  display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer',
                  fontSize: '1.25rem', color: 'var(--ink-20)',
                }}>
                  {imageUploading ? '…' : '+'}
                  <input type="file" accept="image/*" style={{ display: 'none' }} onChange={e => uploadBrandImage(e.target.files?.[0])} disabled={imageUploading} />
                </label>
              )}
            </div>

            {imageError && <p style={{ fontSize: '0.78rem', color: 'var(--failed)', marginBottom: 10 }}>{imageError}</p>}

            {brandImages.length > 0 && (
              <button
                type="button"
                onClick={analyzeBrandImages}
                disabled={analyzing}
                style={{
                  padding: '8px 16px', fontSize: '0.8125rem', fontWeight: 500,
                  background: 'var(--fog)', border: '1px solid var(--fog-60)', borderRadius: 'var(--radius-sm)',
                  cursor: analyzing ? 'default' : 'pointer', marginBottom: 12,
                }}
              >
                {analyzing ? 'Analyzing…' : '🎨 Analyze visual style'}
              </button>
            )}

            {visualStyle && (
              <div style={{ padding: '12px 14px', background: 'var(--fog)', borderRadius: 'var(--radius-sm)', fontSize: '0.8125rem', color: 'var(--ink-40)', lineHeight: 1.5 }}>
                {visualStyle}
              </div>
            )}
          </BrandSection>
