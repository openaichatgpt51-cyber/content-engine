...
export async function getNotifications(supabase) {
  ...
  for (const post of failedPosts || []) {
    ...
    notifications.push({
      ...
      // Review Queue only ever loads AWAITING_APPROVAL posts — a
      // FAILED/PARTIAL post will never appear there, making that link a
      // dead end. The Calendar's detail panel isn't status-restricted and
      // now supports a ?postId= deep link, so send it there instead.
      actionHref:  `/dashboard?postId=${post.id}`,
  ...
  if (sub && sub.posts_used >= sub.posts_limit) {
    notifications.push({
      ...
      time:     null,
      actionHref:  '/dashboard/settings?tab=billing',
